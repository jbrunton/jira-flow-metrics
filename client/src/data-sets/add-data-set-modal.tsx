...
export const AddDataSetModal: React.FC<AddDataSetModalParams> = ({ isOpen, close, domainId }) => {
  ...
  const notFoundContent = dataSourceQuery.trim().length > 0
    ? <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="No data sources found" />
    : <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="Type to search" />;

return <Modal title="Add Domain" open={isOpen} onOk={onSubmit} onCancel={close} confirmLoading={createDataSet.isLoading}>
    <Form form={form}>
      <Form.Item
        name="data-source"
        label="Data source"
        rules={[{ required: true }]}
      >
        <Select showSearch onSearch={setDataSourceQuery} onChange={onSelectDataSource} filterOption={filterOption} notFoundContent={notFoundContent}>
